...
public class ElasticWrapPanel : WrapPanel
{
    ...
    {
        AffectsMeasure<ElasticWrapPanel>(IsFillHorizontalProperty, IsFillVerticalProperty, ItemSpacingProperty,
            LineSpacingProperty);
        AffectsArrange<ElasticWrapPanel>(IsFillHorizontalProperty, IsFillVerticalProperty, ItemSpacingProperty,
            LineSpacingProperty);
    }
    ...
    protected override Size MeasureOverride(Size constraint)
    {
        ...
        foreach (var child in children)
        {
            ...
            else
            {
                ...
                if (MathHelpers.GreaterThan(curLineSize.U + itemUWithSpacing,
                        uvConstraint.U)) // Need to switch to another line
                {
                    ...
                    if (MathHelpers.GreaterThan(sz.U,
                            uvConstraint.U)) // The element is wider than the constraint - give it a separate line
                    {
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        foreach (var child in children)
        {
            ...
            else
            {
                ...
                // Account for spacing before this item if it's not the first
                double spacingBeforeItem = curLineUIs.Count > 0 ? itemSpacing : 0;
                if (MathHelpers.GreaterThan(curLineUIs.TotalU + spacingBeforeItem + sz.U,
        ...
        if (lineUVCollection.Count > 0)
        {
            ...
            if (isFillU)
            {
                if (itemSetSize.U > 0)
                {
                    ...
                    adaptULength = (uvFinalSize.U - maxElementCount * itemSetSize.U - totalItemSpacing) /
                                   maxElementCount;
            ...
            if (isFillV)
            {
                ...
                {
                    isAdaptV = true;
                    double totalLineSpacing =
                        lineUVCollection.Count > 1 ? (lineUVCollection.Count - 1) * lineSpacing : 0;
    ...
    public static readonly DirectProperty<ElasticWrapPanel, int> LineCountProperty =
        AvaloniaProperty.RegisterDirect<ElasticWrapPanel, int>(
