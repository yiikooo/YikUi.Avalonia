...
public class IPv4Box : TemplatedControl
{
    ...
    public static readonly StyledProperty<IPAddress?> IPAddressProperty =
        AvaloniaProperty.Register<IPv4Box, IPAddress?>(
    ...
    public static readonly StyledProperty<IPv4BoxInputMode> InputModeProperty =
        AvaloniaProperty.Register<IPv4Box, IPv4BoxInputMode>(
            nameof(InputMode));
    ...
    static IPv4Box()
    {
        ...
        TextInputMethodClientRequestedEvent.AddClassHandler<IPv4Box>((tb, e) => { e.Client = tb._imClient; });
    }
    ...
    public static KeyGesture? CopyKeyGesture { get; } =
        Application.Current?.PlatformSettings?.HotkeyConfiguration.Copy.FirstOrDefault();

    public static KeyGesture? PasteKeyGesture { get; } =
        Application.Current?.PlatformSettings?.HotkeyConfiguration.Paste.FirstOrDefault();

    public static KeyGesture? CutKeyGesture { get; } =
        Application.Current?.PlatformSettings?.HotkeyConfiguration.Cut.FirstOrDefault();
    ...
    private void ParseBytes(bool showLeadingZero)
    {
        ...
        if (string.IsNullOrEmpty(_firstText?.Text) && string.IsNullOrEmpty(_secondText?.Text) &&
            string.IsNullOrEmpty(_thirdText?.Text) && string.IsNullOrEmpty(_fourthText?.Text))
        {
            _firstByte = null;
            _secondByte = null;
            _thirdByte = null;
