...
public class PopConfirm : ContentControl
{
    ...
    static PopConfirm()
    {
        AvaloniaPropertyExtension.AffectsPseudoClass<PopConfirm>(IsDropdownOpenProperty,
            PC_DropdownOpen);
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        RoutedEventExtension.AddHandler(Button.ClickEvent, OnButtonClicked, _closeButton, _cancelButton,
            _confirmButton);
    }
    ...
    private void OnButtonClicked(object? sender, RoutedEventArgs e)
    {
        ...
        if (sender is Button { Command: { } command and (INotifyPropertyChanged or IDisposable) } button)
        {
            ...
            void OnCanExecuteChanged(object? _, EventArgs a)
            {
                count++;
                if (count < 2) return;
    ...
    protected override bool RegisterContentPresenter(ContentPresenter presenter)
    {
        ...
        if (result)
            _childChangeDisposable =
                ObservableExtensions.Subscribe(presenter.GetPropertyChangedObservable(ContentPresenter.ChildProperty),
                    OnChildChanged);
        return result;
    }
    ...
    private void SetupChildrenEventSubscriptions(Control? child, PopConfirmTriggerMode mode)
    {
        ...
        if (mode.HasFlag(PopConfirmTriggerMode.Click))
        {
            if (child is Button button)
                RoutedEventExtension.AddHandler(Button.ClickEvent, OnMainButtonClicked, button);
            else
                RoutedEventExtension.AddHandler(PointerPressedEvent, OnMainElementPressed, child);
        }

        if (mode.HasFlag(PopConfirmTriggerMode.Focus))
        {
            RoutedEventExtension.AddHandler(GotFocusEvent, OnMainElementGotFocus, child);
            RoutedEventExtension.AddHandler(LostFocusEvent, OnMainElementLostFocus, child);
        }
    }
    ...
    {
        if (child is null) return;
        RoutedEventExtension.RemoveHandler(PointerPressedEvent, OnMainElementPressed, child);
        RoutedEventExtension.RemoveHandler(Button.ClickEvent, OnMainButtonClicked, child);
        RoutedEventExtension.RemoveHandler(GotFocusEvent, OnMainElementGotFocus, child);
        RoutedEventExtension.RemoveHandler(LostFocusEvent, OnMainElementLostFocus, child);
    }
