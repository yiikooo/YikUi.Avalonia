...
internal sealed class Polynomial
{
    ...
    internal Polynomial AddOrSubtract(Polynomial other)
    {
        if (Primitive != other.Primitive)
        {
            throw new ArgumentException(
                $"{nameof(Polynomial)} cannot perform {nameof(AddOrSubtract)} as they do not have the same {nameof(Primitive)}" +
    ...
    internal Polynomial CoefficientXor(int[] smallerCoefficients, int[] largerCoefficients)
    {
        ...
        {
            throw new ArgumentException(
                $"Cannot perform {nameof(CoefficientXor)} method as smaller {nameof(Coefficients)} length is greater than the larger one.");
        }
    ...
    internal Polynomial Multiply(Polynomial other)
    {
        ...
        {
            throw new ArgumentException(
                $"{nameof(Polynomial)} cannot perform {nameof(Multiply)} as they do not have the same {nameof(Primitive)}" +
                $" for {nameof(GaloisField256)}.");
        }
    ...
    internal PolyDivideStruct Divide(Polynomial other)
    {
        ...
        {
            throw new ArgumentException(
                $"{nameof(Polynomial)} cannot perform {nameof(Divide)} as they do not have the same {nameof(Primitive)}" +
                $" for {nameof(GaloisField256)}.");
        }
            ...
            for (int aIndex = 0; aIndex <= aLength - bLength; aIndex++)
            {
                if (aCoefficients[aIndex] != 0)
                {
                    ...
                    if (bCoefficient[0] != 0)
                    {
                        for (int bIndex = 0; bIndex < bLength; bIndex++)
                        {
                            aCoefficients[aIndex + bIndex] =
                                GField.Subtraction(aCoefficients[aIndex + bIndex], bCoefficient[bIndex]);
                        }
                    }
                }
            }
